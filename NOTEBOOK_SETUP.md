...
앞으로 카탈로그(잠금 안쪽 콘텐츠 + 다운로드 링크)는 **노트북에서만** 관리한다.
운영 PC 는 만지지 않는다.
...
```powershell
[Environment]::SetEnvironmentVariable('HP_CATALOG_PW', '0219', 'User')
```
...
- **카드 하나 추가** (URL 또는 파일에서):
  ```powershell
  python add_card.py --url "https://tesla.hyt.kr/o/397a2c29c488f053de2d/hesla-card.txt" --pw 0219
  ```
  또는
  ```powershell
...
- **문구 치환**:
  ```powershell
  python catalog_edit.py replace "옛문구" "새문구" --pw 0219
  ```

- **현재 내용 확인** (복호화):
  ```powershell
  python catalog_edit.py decrypt --pw 0219 | Out-File check.html -Encoding utf8
...
- **특정 키워드 검색**:
  ```powershell
  python catalog_edit.py grep "Hesla" --pw 0219
  ```
...
브라우저에서 https://hyt.kr/hp 열고 `0219` 입력 → 추가한 카드 확인.
...
푸시 후 https://hyt.kr/hp 접속 → `0219` → Hesla 카드 확인 → 다운로드 버튼 클릭 → `hesla.apk` 59MB 받아지면 성공.
